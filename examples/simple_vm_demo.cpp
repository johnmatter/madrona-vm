...
int main(int argc, char* argv[]) {
  try {
    ...
    // unsigned int selectedDevice = ui::DeviceSelector::selectAudioDevice();
    // if (selectedDevice == static_cast<unsigned int>(-1)) {
        // std::cerr << "No audio device selected. Exiting." << std::endl;
        // return 1;
    // }
    // use default device
    unsigned int selectedDevice = AudioDeviceManager::getDefaultOutputDevice();
    // === STEP 1: Load patch file ===
    std::string patch_path = (argc > 1) ? argv[1] : "examples/a440.json";
    std::cout << "1) Loading patch file: " << patch_path << std::endl;
    ...
    std::cout << "2) Parsing JSON into PatchGraph..." << std::endl;
    ...
    std::cout << "3) Compiling PatchGraph to bytecode..." << std::endl;
    ...
    // === STEP 4: Load into VM ===
    std::cout << "4) Loading bytecode into VM..." << std::endl;
    madronavm::VM vm(registry, 44100.0f);
    vm.load_program(std::move(bytecode));
    std::cout << "  - Bytecode successfully loaded into VM\n" << std::endl;
    // === STEP 5: Play audio ===
    std::cout << "5) Playing audio for 5 seconds..." << std::endl;
    // Create the audio driver and link it to the VM
    AudioOut audio_out_driver(44100.0f, false, selectedDevice); // false = not test mode
