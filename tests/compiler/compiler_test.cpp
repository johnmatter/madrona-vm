...
TEST_CASE("Compiler correctly generates bytecode", "[compiler]") {
    ...
    // 2. Compile it
    auto graph = madronavm::parse_json(str);
    auto bytecode = madronavm::Compiler::compile(graph);
    ...
    std::vector<uint32_t> expected_instructions = {
        // Node 1: sine_osc (ID 0x100)
        (uint32_t)madronavm::OpCode::LOAD_K, 0, freq_as_u32,          // LOAD_K r0, 440.0
        (uint32_t)madronavm::OpCode::PROC, 0x100, 1, 1, 0, 1,          // PROC sine_osc in:[r0] out:[r1]
        // Node 2: gain (ID 0x401)
        (uint32_t)madronavm::OpCode::LOAD_K, 2, gain_as_u32,           // LOAD_K r2, 0.5
        (uint32_t)madronavm::OpCode::PROC, 0x401, 2, 1, 1, 2, 3,       // PROC gain in:[r1, r2] out:[r3]
        // Node 3: audio_out (ID 0x001)
        (uint32_t)madronavm::OpCode::PROC, 0x001, 2, 0, 3, 3,          // PROC audio_out in:[r3, r3]
        // End of program
